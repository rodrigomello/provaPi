...
figure(1), colormap(gray(256)),image(f), title('Imagem Ruidosa');

figure(2),colormap(gray(256)), image(abs(f_passAlta)), title('Imagem Ruidosa com passa alta');

figure(3),colormap(gray(256)),  image(f_passBaixa), title('Imagem Ruidosa com passa baixa');

figure(4),colormap(gray(256)), image(f_passAlta2), title('Imagem passa baixa/passa alta');
